...
export class CryptoManager {
    ...
    private constructor(configPath: string) {
        ...
        if (!config.encryptionKey) {
            this.loggerService.info("Nie znaleziono encryptionKey w config. Generuję nowy...")
            config.encryptionKey = this.generateKey(configPath);
        }
    ...
    public encryptCardData(data: CardData): Buffer {
        ...
        if (uuidBuffer.length !== 16) throw new AppError("UUID musi mieć 16 bajtów.");
        const encrypted = Buffer.concat([cipher.update(uuidBuffer), cipher.final()]);
        this.loggerService.info("Zaszyfrowano UUID karty do 16 bajtów.")
        return encrypted; // teraz dokładnie 16 bajtów
    }

    /** Odszyfrowanie UUID */
    public decryptCardData(payload: Buffer): CardData {
        if (payload.length !== 16) throw new AppError("Zaszyfrowane dane muszą mieć 16 bajtów.");
        ...
        this.loggerService.info("Odszyfrowano UUID karty.")
        return { uuid } as CardData;
    }


    /** Generowanie nowego klucza 128-bitowego i zapis do config */
    public generateKey(configPath: string): string {
        ...
        this.loggerService.warn("Wygenerowano nowy klucz szyfrowania UUID.")
